...
describe("Utility Functions Tests", () => {
  ...
  test.only("averageWordLength correctly calculates the average length of words in an array", () => {
    const words = ["eclipse", "harmony", "cascade"];
    expect(averageWordLength(words)).toBeCloseTo(7, 0);
    expect(averageWordLength([])).toBeNull();
    expect(averageWordLength(null)).toBeNull();
  });
  ...
  test("uniquifyArray correctly removes duplicates from an array", () => {
    ...
    expect(uniquifyArray(words)).toEqual([
      "crab",
      "poison",
      "contagious",
      "simple",
